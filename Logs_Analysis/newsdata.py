...
def topauthors():
    ...
    query2 = """ create view topauthors as
                    ...
                    where '/article/' || articles.slug = log.path and articles.author = authors.id and authors.name != 'Anonymous Contributor'
                    group by name
                    order by num desc
                    limit 3;
...
for row in topauthors():
    stat = str(num2) + '.) ' + str(row[0]) + ": " + str(('{:,}'.format(row[1]))) + ' views'
    print(stat)
    num2 += 1
...
for row in errorsperc():
    ...
    stat = str(row[0]) + ": " + str(round(decimal.Decimal(row[1]), 2)) + '%' + ' errors'
    print(stat)
